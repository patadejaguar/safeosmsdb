...
SELECT
		...
		MIN(IF(`afectacion_real`>0 AND `tipo_operacion` = 410,`periodo_socio`,CS_.`pagos_autorizados`))	AS `letra_minima`,
		...
		MIN(IF(`afectacion_real`>0 AND `tipo_operacion` = 410,`fecha_afectacion`,CS_.`fecha_vencimiento`))			AS `fecha_primer_atraso`

		FROM
			`operaciones_mvtos` `operaciones_mvtos` 
				INNER JOIN `creditos_solicitud` CS_ 
				ON `operaciones_mvtos`.`docto_afectado` = CS_.
				`numero_solicitud` 
					INNER JOIN `eacp_config_bases_de_integracion_miembros` 
					`eacp_config_bases_de_integracion_miembros` 
					ON `operaciones_mvtos`.`tipo_operacion` = 
